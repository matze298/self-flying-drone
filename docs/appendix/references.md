# References

Primary sources are preferred. Verify current board revisions, software releases, regulatory text, and regional radio rules before purchase or flight.

## Documentation-site stack

- [uv documentation](https://docs.astral.sh/uv/)
- [MkDocs documentation](https://www.mkdocs.org/)
- [Material for MkDocs — publishing](https://squidfunk.github.io/mkdocs-material/publishing-your-site/)
- [Material for MkDocs — versioning](https://squidfunk.github.io/mkdocs-material/setup/setting-up-versioning/)

## Development stack

- [ROS 2 release policy / REP 2000](https://raw.githubusercontent.com/ros-infrastructure/rep/master/rep-2000.rst)
- [ROS distribution index](https://raw.githubusercontent.com/ros/rosdistro/master/index-v4.yaml)
- [colcon documentation](https://colcon.readthedocs.io/en/released/)
- [NVIDIA JetPack documentation](https://developer.nvidia.com/embedded/jetpack)
- [ros2_rust / rclrs](https://github.com/ros2-rust/ros2_rust)
- [GStreamer Rust bindings](https://gitlab.freedesktop.org/gstreamer/gstreamer-rs)
- [Rust MAVLink implementation](https://github.com/mavlink/rust-mavlink)

## Flight systems

- [ArduPilot companion computers](https://ardupilot.org/dev/docs/companion-computers.html)
- [ArduPilot MAVLink basics](https://ardupilot.org/dev/docs/mavlink-basics.html)
- [ArduPilot Plane geofencing](https://ardupilot.org/plane/docs/geofencing.html)
- [ArduPilot Plane advanced failsafe](https://ardupilot.org/plane/docs/advanced-failsafe-configuration.html)
- [Matek H743 Wing support in ArduPilot Plane](https://ardupilot.org/plane/docs/common-matekh743-wing.html)
- [MAVSDK](https://github.com/mavlink/mavsdk)

## Compute, vision, and video

- [NVIDIA Jetson Orin Nano developer-kit documentation](https://docs.nvidia.com/jetson/orin-nano-devkit/user-guide/latest/)
- [Jetson Orin Nano series power and performance](https://docs.nvidia.com/jetson/archives/r39.2/DeveloperGuide/SD/PlatformPowerAndPerformance/JetsonOrinNanoSeriesJetsonOrinNxSeriesAndJetsonAgxOrinSeries.html)
- [OpenCV license](https://opencv.org/license/)
- [ONNX Runtime](https://onnxruntime.ai/)
- [Ultralytics licensing note](https://docs.ultralytics.com/help/contributing/)
- [OpenHD source repository](https://github.com/OpenHD/OpenHD)

## EU / Germany operations

- [EASA drone FAQs](https://www.easa.europa.eu/en/the-agency/faqs/drones-uas)
- [EASA: automatic vs autonomous drone operations](https://www.easa.europa.eu/en/the-agency/faqs/regulations-uas-drone-explained)
- [EASA Easy Access Rules for UAS](https://www.easa.europa.eu/en/document-library/easy-access-rules/easy-access-rules-unmanned-aircraft-systems)
- [Luftfahrt-Bundesamt: A1/A3 competency](https://www.lba.de/DE/Drohnen/Fernpiloten/Kompetenznachweis_A1_A3.html)
